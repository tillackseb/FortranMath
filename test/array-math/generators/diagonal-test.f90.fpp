...
module m_diagonal_test
  ...
  #: for t in INTEGER_TYPES[0:1]
  public :: diagonal_${t['name']}$_test
  #: endfor
...
contains
  ...
  !> and [[m_diagonal(module):get_diagonal(interface)]] with arrays of `type ${t['type']}$(kind=${t['kind']}$)`.
  subroutine diagonal_${t['name']}$_test( error )
    ...
    integer :: isub, i
    ${t['type']}$(kind=${t['kind']}$) :: diag_put(n), M(shapeM(1), shapeM(2), shapeM(3))
    ${t['type']}$(kind=${t['kind']}$), allocatable :: diag_get(:), A(:,:), Msub(:,:,:)
    ...
    do isub = 1, size( sub_diag )
      ...
      call check( error, &
        ...
        '`diagonal` failed for allocatable rank 2 array of type `${t['type']}$(kind=${t['kind']}$)`.', &
      ...
      call check( error, &
        ...
        '`diagonal` failed for allocatable rank 1 array of type `${t['type']}$(kind=${t['kind']}$)`.', &
        'Returns incorrect diagonal element.' )
      if (allocated(error)) return
    end do
    ! check for rank 3
    ...
    call check( error, &
      ...
      '`diagonal` failed for rank 3 array of type `${t['type']}$(kind=${t['kind']}$)`.', &
      'Returns incorrect diagonal element.' )
    if (allocated(error)) return
  ...
  end subroutine diagonal_${t['name']}$_test
